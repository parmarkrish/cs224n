...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[torch.Tensor, Tuple[torch.Tensor, torch.Tensor]]:
        ...
        enc_hiddens = torch.permute(enc_hiddens, [1, 0, 2])  # (src_len, b, 2h) -> (b, src_len, 2h)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1):
            Y_t = torch.squeeze(Y_t, 0) # (1, b, e) -> (b, e)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        dec_hidden, _ = dec_state = self.decoder(Ybar_t, dec_state)
        e_t = torch.squeeze(torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden, 2)), 2)  # (b, src_len, h) @ (b, h, 1) -> (b, src_len, 1) -> (b, src_len)
        ...
        alpha_t = F.softmax(e_t, 1)  # (b, src_len) -> (b, src_len)
        alpha_t = torch.unsqueeze(alpha_t, 1) # (b, src_len) -> (b, 1, src_len) 
        a_t = torch.squeeze(torch.bmm(alpha_t, enc_hiddens), 1) # (b, 1, src_len) @ (b, src_len, 2h) -> (b, 1, 2h) -> (b, 2h)
