...
\begin{subparts}
    ...
    \ifans{
    $\alpha$ takes on $n$ different categories (corresponding to the $n$ different attention weights) and sums to 1 due to the softmax operation in (2)
    }
    ...
    \ifans{
    The query $q$ and key $k_j$ should be identical or close to identical while the other keys should be dissimilar to $q$ for $\alpha_j$ to be large.
    }
    \subpart[1] Under the conditions you gave in (ii),  \textbf{describe} the output $c$. 
    
    \ifans{The output $c$ would effectively be a copy of $v_j$ as $\alpha_j$ $\approx$ 1 while for $i \neq j$ $\alpha_i \approx 0$.}
    \subpart[1] \textbf{Explain} (in two sentences or fewer) what your answer to (ii) and (iii) means intuitively. \\
    \ifans{
    For the query $q$, the transformer is only focused on one vector from the sequence. It isn't incorporating information from other vectors in the sequence.
    }
...
The rightmost matrix will create the vector 
\begin{bmatrix} 
c_1 \\
c_2 \\
\vdots \\
c_m \\
\end{bmatrix}
which when multiplied with the leftmost matrix, will result in $v_a$.
}
...
\ifans{
Let $q = x(k_a + k_b)$ where $x >> 0$. Then, pre-softmax $\alpha_a = q^\top k_a = x(k_a + k_b)^\top k_a = x$. Similarly $\alpha_b = x$. $\alpha_i = 0$ where $i \neq a,b$. During softmax $\alpha_i = \exp(0) = 1$ where $i \neq a,b$  and $\alpha_a = \alpha_b = \exp(x)$ ($\alpha_a$ and $\alpha_b$ are the terms that dominate the softmax). So, post-softmax, $\alpha_a = \alpha_b = \frac{x}{x + x + 1 + \cdots + 1} \approx 0.5$ for large enough x. Thus when computing the weighted sum of the values, we get an average between $v_a$ and $v_b$.}
}
\end{subparts}
...
\ifans{We let $q = x(\mu_a + \mu_b)$ where $x >> 0$. Now $k_i \approx \mu_i$ since the covariance is small due to the vanishingly small $\alpha$. We can thus apply the same reasoning from the above question since $\mu_i$ is unit norm and orthogonal to $\mu_j$ where $i \neq j$.}
...
\ifans{$k_a$ will be roughly in the same direction as $\mu_a$ but will likely have a larger or small magnitude. So, when performing the following dot product $q^\top k_a = x(\mu_a + \mu_b)^\top \beta\mu = \beta x$ where $\beta$ is the factor by which $k_a$ is larger or smaller than $\mu_a$, the weighting in $c$'s average of $v_a$ and $v_b$ will differ due to the variation in $k_a$'s magnitude. So $c$ will oscillate between being weighted towards $v_a$ and being weighted towards $v_b$.
}
\end{subparts}
...
\ifans{
Let $q_1 = x\mu_a$ and $q_2 = x\mu_b$ where $x >> 0$. Then $c_1 \approx v_a$ and $c_2 \approx v_b$ due to a similar reason as above. $c = \frac{1}{2}(c_1 + c_2) \approx \frac{1}{2}(v_a + v_b)$
}
...
\ifans{
I believe that $c \approx \frac{1}{2}(v_a + v_b)$ since $c_1$ and $c_2$ won't have change significantly. For large x $c_1$ will still be about equal to $v_a$. The dot product $q_a^\top k_a$ will get affected due to the variance in magnitude of $k_a$, but will still be a large number on the order of to $x$ (e.g, it will be $\beta x$ where $k_a = \beta \mu_$, $\beta$ accounts for the magnitude of $k_a$). Hence $c_a \approx v_a$. On the other hand, $c_2$ will not be affected. Thus $c \approx \frac{1}{2}(v_a + v_b)$ even due to the variance of magnitude from $k_a$
}
