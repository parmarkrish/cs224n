...
\begin{answer}
\begin{align*}
\frac{\partial \bm J_{naive-softmax}}{\partial \bm v_c} 
&= \frac{\partial}{\partial \bm v_c} \left[-\log \left(\frac{\exp(\bm u_o^\top \bm v_c)}{\sum_{w \in \text{Vocab}} \exp(\bm u_{w}^\top \bm v_c)} \right) \right]\\
&= -\frac{\partial}{\partial \bm v_c} \left[\bm u_o^\top \bm v_c -log \left(\sum_{w \in Vocab} \exp(\bm u_w^\top \bm v_c) \right) \right]\\
&= -\bm u_o + \frac{1}{\sum_{w' \in Vocab} \exp(\bm u_{w'}^\top \bm v_c)} \sum_{w \in Vocab}\frac{\partial}{\partial \bm v_c} \exp(\bm u_w^\top v_c)\\
&= -\bm u_o +\sum_{w \in Vocab} \frac{\bm u_w \exp(\bm u_w^\top v_c)}{\sum_{w' \in Vocab} \exp(\bm u_{w'}^\top \bm v_c)}\\
&= -\bm u_o + \sum_{w \in Vocab}\hat{y}_w\bm u_w\\
&= -\bm U y + \bm U \hat y\\
&= \bm U(\hat{\bm y} - \bm y)
\end{align*}
(1) The gradient is zero when the predicted distribution $\hat{\bm y}$ is equal to the true distribution $\bm y$.\\
(2) By subtracting this gradient from $\bm v_c$, we update $\bm v_c$ so as to slightly minimize the loss. This  means that $\hat{\bm y}$ would be closer to $\bm y$ and a greater dot between the 'outside' and 'center' word vectors is achieved (e.g., the vectors come closer together).
\end{answer}
\end{shaded}
...
\\(i)
\begin{align*}
\frac{\partial \bm J_{neg-sample}}{\partial \bm v_c}
&=  -\frac{1}{\sigma(\bm u_o^\top \bm v_c)}\left(\sigma(\bm u_o^\top \bm v_c)(1 - \sigma(\bm u_o^\top \bm v_c) \right) \bm u_o -\sum_{s=1}^k \frac{1}{\sigma(-\bm u_{w_s}^\top \bm v_c)} \left(\sigma(-\bm u_{w_s}^\top \bm v_c)(1-\sigma(-\bm u_{w_s}^\top \bm v_c))\right) \bm (-\bm u_{w_s}^\top)\\
&= (\sigma(\bm u_o^\top \bm v_c) - 1)\bm u_o - \sum_{s=1}^k(\sigma(-\bm u_{w_s}^\top \bm v_c) - 1)\bm u_{w_s}\\
\frac{\partial \bm J_{neg-sample}}{\partial \bm u_o}
&= -\frac{1}{\sigma(\bm u_o^\top \bm v_c)} \left( \sigma(\bm u_o^\top \bm v_c)(1 - \sigma(\bm u_o^\top \bm v_c))\right)\bm v_c\\
&= (\sigma(\bm u_o^\top \bm v_c)-1)\bm v_c\\
\frac{\partial \bm J_{neg-sample}}{\partial \bm u_{w_s}}
&= -\frac{1}{\sigma(-\bm u_{w_s}^\top \bm v_c)}\left(\sigma(-\bm u_{w_s}^\top \bm v_c)(1-\sigma(-\bm u_{w_s}^\top \bm v_c))\right)(-\bm v_c)\\
&= -(\sigma(-\bm u_{w_s}^\top \bm v_c) - 1)\bm v_c
\end{align*}
(ii)
\begin{align*}
\sigma(\bm{U}_{o, \{w_1, \dots, w_K\}}^\top \bm v_c) - \bm{1} =
\begin{bmatrix}
...
\end{align*}
(iii)
Instead of summing through the entire vocabulary, we only sum $K$ elements where $K << |V|$.
\end{answer}
\end{shaded}
...
\begin{align*}
\frac{\partial \bm J_{neg-sample}}{\partial \bm u_{w_s}}
&= \frac{\partial}{\partial \bm u_{w_s}} \left[ -\log(\sigma(\bm u_o^\top \bm v_c)) - \left(\sum_{\substack{1 \le s' \le K\\ w_{s'} = w_{s}}} \log(\sigma(-\bm u_{w_{s'}}^\top \bm v_c)) + \sum_{\substack{1 \le s' \le K \\ w_{s'} \neq w_{s}}} \log(\sigma(-\bm u_{w_{s'}}^\top \bm v_c))\right) \right]\\
&= -\sum_{\substack{1 \le s' \le K\\ w_{s'} = w_{s}}} (\sigma(-\bm u_{w_{s'}}^\top \bm v_c) - 1)\bm v_c
\end{align*}
\end{answer}
\end{shaded}
