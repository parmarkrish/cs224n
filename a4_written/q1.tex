...
\end{lstlisting}
    ...
    \ifans {
        Positions in $e_t$ that correspond to a 1 in the mask are set to $-\infty$. When we compute softmax, these values will not effect the softmax computation as $\exp{-\infty} = 0$. And consequently, these values will not effect the attention computation. \\
        It is necessary to use masks in in this way as the values corresponding to 1 in the mask are just pad values and not actually part of the sequence.
    } \newline
\end{parts}
...
\end{lstlisting}
    ...
    \ifans{
     (did not train extensively, so my BLEU score is below 10)
    } \newline
    ...
    \ifans{
        \begin{itemize}
            \item Advantage: Dot product attention is more simpler than multiplication attention, and thus requires less parameters (since we aren't using a matrix).
            \item Disadvantage: We are using all the information present in the source and target hidden states which may not be useful in computing our attention scores. With multiplicative attention, the weight matrix can select important features useful for attention.
        \end{itemize}
    } 
    \subpart[2] Explain one advantage and one disadvantage of \textit{additive attention} compared to multiplicative attention. \\
    \ifans{
        \begin{itemize}
            \item Advantage: The source and target hidden states go through a neural-net layer with non-linearity, allowing the function to be more expressive and complex.
            \item Disadvantage: Additive attention can be slow due to the non-linearity. Multiplicative attention tends to be much faster as it is just matrix multiplication.
        \end{itemize}
