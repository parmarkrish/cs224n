...
\begin{parts}
    ...
    \ifans{
        In a polysynthetic language, words can by quite long and complex as they may consist of many affixes to express syntactic and semantic meaning. A single word may contain an entire sentence's worth of meaning. If we choose to model our NMT problem at the word-level, we would have a combinatorially large number of words to deal with.
    } \fi \\
    ...
    \ifans{
        By training on transliterated Cherokee text, we can train on the smallest units of meaning, morphemes. For example, in Cherokee, morphemes such as "ts-" are quite common. If we train on Cherokee characters such as {\cherokeefam Ꮳ} ("tsa"), we may miss out on these common morphemes which are crucial to the meaning of a word.
    } \fi \\
    ...
    \ifans{
          A multilingual model just knows a lot about how language works. Like syntactic and semantic relationships between a lot of languages. Therefore a low-resource language can partially share a representation with other linguistically similar languages, thus improving performance. In other words, multilingual models can draw on context (from other languages) to help out low-resource languages.
    } \fi \\
    ...
    \begin{subparts}
        ...
        \ifans{
        \begin{itemize}
            \item Error: The model incorrectly translated the gender pronouns (twice).
            \item Reason(s): Personal pronouns may be expressed through morphemes not present as a Cherokee character or sequence of characters (i.e. a prefix).
            \item Fix: Train on English transliterated sub-words (as expressed in $b$).
        \end{itemize}
        }
        \subpart[2]
        ...
        \ifans{
        \begin{itemize}
            \item Error: The model erroneously repeated 'little' multiple times.
            \item Reason(s): The beam size of the model may be to small, leading to similar behaviors of greedy decoding.
            \item Fix: Increase the beam size of the model.
        \end{itemize}
        }
        ...
        \ifans{
        \begin{itemize}
            \item Error: The model incorrectly translated 'Humble'.
            \item Reason(s): The model may not have enough representation power to map a large sequence of words to a single word in English.
            \item Fix: Increase hidden state size of model.
        \end{itemize}
        }
    \end{subparts}
    ...
    \begin{subparts}
        ...
        \newline
        \ifans{
            NMT Translation $\bc_1$:
        \begin{align*}
            p_1 &= \frac{2 (\text{and}) + 3 (\text{the}) + 1 (\text{light}) + 1 (\text{shines}) + 1 (\text{in}) + 2 (\text{darkness}) + 0 (\text{can}) + 1 (\text{not}) + 1 (\text{comprehend})}{13} \\
            &= \frac{12}{13}
        \end{align*}

        \begin{align*}
            p_2 &= \frac{\begin{aligned}
                &2 (\text{and the}) + 1 (\text{the light}) + 1 (\text{light shines}) \\
                &+ 1 (\text{shines in}) + 1 (\text{in the}) + 2 (\text{the darkness}) \\
                &+ 1 (\text{darkness and}) + 0 (\text{darkness can}) + 0 (\text{can not}) \\
                &+ 1 (\text{not comprehend})
              \end{aligned}}{12} \\
            &= \frac{10}{12}
        \end{align*}
        ...
        \begin{align*}
            ...
            &= 1 \times \exp \Big( 0.5 \log \frac{12}{13} + 0.5 \log \frac{10}{12} \Big) \\
            &= 0.88
        \end{align*}
        ...
        \ifans{
        Translation $\bc_1$ is considered a better translation is it has a higher BLEU score. I agree that it
        does have a better translation
        }
        ...
        \ifans{
            NMT Translation $\bc_1$:
        \begin{align*}
            p_1 &= \frac{1 (\text{and}) + 3 (\text{the}) + 1 (\text{light}) + 1 (\text{shines}) + 1 (\text{in}) + 2 (\text{darkness}) + 0 (\text{can}) + 1 (\text{not}) + 0 (\text{comprehend})}{13} \\
            &= \frac{10}{13}
        \end{align*}
        ...
        The second translation achieves a better score now. I disagree with this conclusion
        }
        
        \subpart[2] Due to data availability, NMT systems are often evaluated with respect to only a single reference translation. Please explain (in a few sentences) why this may be problematic. In your explanation, discuss how the BLEU score metric assesses the quality of NMT translations when there are multiple reference transitions versus a single reference translation.

        \ifans{
        In the calculation of the BLEU score, we take a max of a count of an n-gram over the multiple reference translations. This means we take "whatever best fits" from the multiple reference translations. If we do not have enough reference translations, our translation may be valid, but different from the reference translation, thus causing a low BLEU score. I.e. we don't get multiple opportunities to match our translation since we only have one reference translation.
        }
        
        \subpart[2] List two advantages and two disadvantages of BLEU, compared to human evaluation, as an evaluation metric for Machine Translation.

        \ifans{\\
        Advantage 1: BLEU quantifies how "good" the translation is. It gives a score instead of relying on a subjective assesment.

        Advantage 2: BLEU is fast to compute. A human evaluation is slow and labourious and can't be scaled.

        Disadvantage 1: If there are not many reference translations, the BLEU score can be an inaccurate measure of performance.

        Disadvantage 2: BLEU score is based off of n-gram overlap. It doesn't consider the meaning or flow of a sentence So, in theory, a translation could have a high BLEU score but be of low quality.
        }
