...
\begin{parts}
    ...
    \part[4] Adam Optimizer\newline
        ...
        \begin{subparts}
            ...
            \ifans{
            Since we are calculating the loss with respect to a mini-batch, our gradient will be a very noisy estimate of the true gradient. By using $\bm$, we can average our estimated gradient over multiple time steps, thus giving us a much better and less noisy estimate of the true gradient.
            }\newline
            ...
            \ifans{
            One problem with standard gradient descent is that the component of the gradient in steep directions will be large, which may lead to oscillation in those directions, and the component of the gradient in shallow direction will be small, leading to slow descent. By dividing by $\sqrt{\bv}$, we slow down the gradient in steep directions (since $\bv$ will be relatively large), and speed up the gradient in shallow directions (since $\bv$ will be relatively small), leading to easier/quicker descent.
            }\newline
    ...
    \part[4] 
        ...
        \begin{subparts}
            ...
            \ifans{
            $\gamma$ must be $1/p_{\text{drop}}$. For example, if we drop a unit with a probability of $.3$, we need to upscale all the remaining units by $1/.3 = 1.\Bar{3}$ in order to get the same expected value.
            }\newline
            ...
            \ifans{
            During training time, dropout forces the network to have a redundant representation as some units may be dropped. This increases the performance as the network will base its predictions on all features.
            
            During evaluation, we would like to use all of our neurons (to fully encapsulate the redundant representation created at training time) and to make our evaluation deterministic.
            }\newline
