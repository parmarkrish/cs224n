...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # get a chunk and pad it so that the total size is block_size + 1
        chunk = self.data[idx] + self.PAD_CHAR * (1 + self.block_size - len(self.data[idx]))

        truncate_length = random.randint(4, int(self.block_size*7/8))
        truncated_chunk = chunk[:truncate_length]

        masked_content_length = random.randint(0, int(truncate_length*1/2))
        masked_content_start = random.randint(0, truncate_length - masked_content_length)

        prefix = truncated_chunk[0:masked_content_start]
        masked_content = truncated_chunk[masked_content_start:masked_content_start+masked_content_length]
        suffix = truncated_chunk[masked_content_start+masked_content_length:]

        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + \
                        self.PAD_CHAR * (1 + self.block_size - truncate_length - 2)
