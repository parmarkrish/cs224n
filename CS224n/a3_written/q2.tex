...
\begin{parts}
    ...
    \begin{tabular}{ l | l | l | l}
    Stack & Buffer & New dependency & Transition \\ \hline
    $[$ROOT, parsed] & [a, sentence] & parsed$\to$Today &  
    \texttt{LEFT-ARC} \\
    $[$ROOT, parsed, a] & [sentence] &  &  
    \texttt{SHIFT} \\
    $[$ROOT, parsed, a, sentence] & [ ] &  &  
    \texttt{SHIFT} \\
    $[$ROOT, parsed, sentence] & [ ] & sentence$\to$a &  
    \texttt{LEFT-ARC} \\
    $[$ROOT, parsed] & [ ] & parsed$\to$sentence &  
    \texttt{RIGHT-ARC} \\
    $[$ROOT] & [ ] & root$\to$parsed &  
    \texttt{RIGHT-ARC} \\
    \end{tabular}\newline
    ...
    \ifans{
    A sentence containing $n$ words will be parsed in $2n$ steps. The parser must eventually shift until the buffer is empty, thus accounting for $n$ steps, and the parser must assign every word a dependency, thus accounting for another $n$ steps.
    }\newline
...
\input{dep_parses.tex}
 ...
 \end{itemize}
  ii.
  \begin{itemize}
    \item \textbf{Error type}: Prepositional Phrase Attachment 
    \item \textbf{Incorrect dependency}: chasing $\rightarrow$ fur
    \item \textbf{Correct dependency}: dogs $\rightarrow$ fur
  \end{itemize}
  iii.
  \begin{itemize}
    \item \textbf{Error type}: Modifier Attachment Error Error 
